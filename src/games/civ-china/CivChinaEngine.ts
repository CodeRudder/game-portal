...
import {
  GAME_ID, GAME_TITLE, BUILDINGS, OFFICIALS, DYNASTIES, INVENTIONS,
...
export interface CivChinaSaveState {
  resources: Record<string, number>;
  buildings: Record<string, number>;
  officials: Record<string, { level: number; exp: number }>;
...
export class CivChinaEngine extends IdleGameEngine {
  ...
  protected onInit(): void {
    ...
    this.stages = new StageSystem<StageDef>(DYNASTIES, 'xia');
  ...
  protected onRender(ctx: CanvasRenderingContext2D, w: number, h: number): void {
    ...
    switch (this.panel) {
      ...
      case 'officials': this.drawOfficials(ctx, w, cy, ch); break;
    }
  ...
  public serialize(): CivChinaSaveState {
    const offs: Record<string, { level: number; exp: number }> = {};
    for (const o of OFFICIALS) {
      const s = this.units.getState(o.id);
      if (s?.unlocked) offs[o.id] = { level: s.level, exp: s.exp };
    }
    return {
      resources: { ...this.res },
  ...
  public deserialize(d: CivChinaSaveState): void {
    this.res = { ...d.resources };
    this.bldg.loadState(d.buildings);
    for (const [id, o] of Object.entries(d.officials)) {
  ...
  public doPrestige(): void {
    const total = (this.res.food || 0) + (this.res.silk || 0) + (this.res.culture || 0);
    ...
    this.res.food = (this.res.food || 0) * ret;
    this.res.silk = (this.res.silk || 0) * ret;
    this.res.culture = (this.res.culture || 0) * ret;

    // 重置建筑和阶段
    this.bldg.reset(true);
    this.stages.loadState({ currentStageId: 'xia' });
  ...
  private checkStage(): void {
    ...
    if (can) {
      ...
      this.ftSys.add(`朝代更替：${next.name}`, 0.5, 0.3, { style: { color: COLOR_THEME.accentGold, fontSize: 22 } });
    }
  }
  ...
  private toUnitDefs() {
    ...
    return OFFICIALS.map(o => ({
      id: o.id, name: o.name, description: o.title,
      rarity: rMap[o.rarity] ?? 0,
      baseStats: { administration: o.baseStats.administration, military: o.baseStats.military, culture: o.baseStats.culture },
      growthRates: { ...o.growthRates },
      evolutions: [],
      recruitCost: Object.entries(o.recruitCost).map(([materialId, quantity]) => ({ materialId, quantity })),
      maxLevel: 50, tags: [o.rarity], passiveSkillIds: [],
    }));
  }

  private makeStatDefs() {
    const ids = [
      'totalFood', 'totalSilk', 'totalCulture', 'totalMandate',
      'totalClicks', 'totalPrestiges', 'totalOfficialsRecruited', 'totalTechsResearched', 'totalPlayTime',
    ];
    return ids.map(id => ({
  ...
  private drawHeader(ctx: CanvasRenderingContext2D, w: number): void {
    ...
    const tabs = [
      ...
      { id: 'officials' as ActivePanel, label: '📜官员' },
      { id: 'tech' as ActivePanel, label: '💡发明' },
      { id: 'prestige' as ActivePanel, label: '👑天命' },
    ];
  ...
  private drawOfficials(ctx: CanvasRenderingContext2D, w: number, sy: number, h: number): void {
    const ih = 65, vis = Math.floor(h / ih);
    for (let i = 0; i < Math.min(OFFICIALS.length, vis); i++) {
      const idx = i + this.scroll;
      if (idx >= OFFICIALS.length) break;
      const o = OFFICIALS[idx], iy = sy + i * ih;
      ...
      ctx.textAlign = 'right';
      if (!own) {
        const costStr = Object.entries(o.recruitCost).map(([k, v]) => `${v}${k === 'food' ? '🌾' : '🧵'}`).join(' ');
  ...
  private drawFooter(ctx: CanvasRenderingContext2D, w: number, h: number): void {
    ...
    ctx.fillText('[Space]点击 [↑↓]选择 [Enter]购买 [R]声望 [T]科技 [U]官员 [Esc]返回', w / 2, h - 12);
    ctx.textAlign = 'left';
  }
