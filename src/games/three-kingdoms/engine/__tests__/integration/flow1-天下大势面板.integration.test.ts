...
describe('v6.0 集成测试 — Flow 1: 天下大势面板 + 急报横幅 + 事件筛选', () => {
  ...
  describe('§1 天下大势面板', () => {
    ...
    it('面板数据应与实际游戏状态一致 — 注册→触发→查询', () => {
      ...
      expect(result.instance!.eventId).toBe('evt-panel-1');
    });
  ...
  describe('§1.1 急报横幅', () => {
    ...
    it('应支持标记全部已读', () => {
      const makeInstance = (id: string): EventInstance => ({
        instanceId: id, eventId: `e-${id}`, triggerTurn: 1, expireTurn: 10, status: 'active',
      });
    ...
    it('应支持移除横幅', () => {
      const instance: EventInstance = {
        instanceId: 'inst-del-1', eventId: 'evt-del-1', triggerTurn: 1, expireTurn: 10, status: 'active',
      };
    ...
    it('应支持横幅过期清理', () => {
      const instance: EventInstance = {
        instanceId: 'inst-exp-1', eventId: 'evt-exp-1', triggerTurn: 1, expireTurn: 5, status: 'active',
      };
    ...
    it('横幅创建应发出事件总线通知', () => {
      const instance: EventInstance = {
        instanceId: 'inst-emit-1', eventId: 'evt-emit-1', triggerTurn: 1, expireTurn: 10, status: 'active',
      };
    ...
    it('应支持dismiss横幅', () => {
      const instance: EventInstance = {
        instanceId: 'inst-dismiss-1', eventId: 'evt-dismiss-1', triggerTurn: 1, expireTurn: 10, status: 'active',
      };
