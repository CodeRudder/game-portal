...
/** 从 createSim 获取真实引擎的事件子系统 */
function getEventSystems() {
  const sim = createSim();
  const registry = sim.engine.getSubsystemRegistry();
  return {
    sim,
    triggerSys: registry.get<EventTriggerSystem>('eventTrigger'),
    notifySys: registry.get<EventNotificationSystem>('eventNotification'),
...
describe('v6.0 集成测试 — Flow 1: 天下大势面板 + 急报横幅 + 事件筛选', () => {
  ...
  describe('§1 天下大势面板', () => {
    ...
    beforeEach(() => {
      const systems = getEventSystems();
      triggerSys = systems.triggerSys;
      notifySys = systems.notifySys;
  ...
  describe('§1.1 急报横幅', () => {
    ...
    it('横幅创建应发出事件总线通知', () => {
      ...
      sim.engine.on('event:banner_created', (data: unknown) => { emittedData = data; });

      notifySys.createBanner(instance, { title: '通知', description: '', urgency: 'high' });
      expect(emittedData).not.toBeNull();
      expect(emittedData).toEqual(expect.objectContaining({ title: '通知', urgency: 'high' }));
    });
