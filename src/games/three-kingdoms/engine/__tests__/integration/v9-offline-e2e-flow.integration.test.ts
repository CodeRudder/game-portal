...
describe('v9.0 E2E — §7.5 离线收益→邮件系统→活动奖励三系统串联', () => {
  ...
  it.skip('§7.5 离线收益弹窗资源数值与公式一致 [引擎未实现]', () => {
    // Play: Step1离线收益弹窗(资源收益) → Step2回归面板(含活动进度摘要) → 邮件系统
    // 引擎未实现邮件系统和活动系统的离线联动
  });

  it.skip('§7.5 三大系统无重复发放、无遗漏 [引擎未实现]', () => {
    // 引擎未实现跨系统去重机制
  });
...
describe('v9.0 E2E — §7.6 邮件过期→清理→补偿全链路', () => {
  ...
  it.skip('§7.6 过期前3天黄色⚠️标签 [UI层测试]', () => {
    // UI层验证
  });

  it.skip('§7.6 过期后自动清理+铜钱50%补偿邮件 [引擎未实现]', () => {
...
describe('v9.0 E2E — §7.7 邮箱满载→暂存→清理→补发全链路', () => {
  ...
  it.skip('§7.7 满载后新邮件进入暂存队列(上限20封) [引擎未实现]', () => {
    // 引擎未实现邮件暂存队列
  });

  it.skip('§7.7 清理后暂存邮件按FIFO顺序补发 [引擎未实现]', () => {
    // 引擎未实现邮件补发
  });
...
describe('v9.0 E2E — §7.11 活动离线→回归面板→邮件→活动面板四系统串联', () => {
  ...
  it.skip('§7.11 赛季活动积分按50%效率累积 [引擎未实现]', () => {
    // Play: 赛季活动50%效率累积积分+代币
    // 引擎未实现活动离线积分
  });

  it.skip('§7.11 限时活动按30%效率累积 [引擎未实现]', () => {
    // 引擎未实现活动离线积分
  });

  it.skip('§7.11 各活动积分独立不混淆 [引擎未实现]', () => {
    // 引擎未实现活动积分系统
  });
...
describe('v9.0 E2E — §7.12 经验离线→升级→邮件→回归面板联动', () => {
  ...
  it.skip('§7.12 离线经验正确计算 [引擎未实现]', () => {
    // Play: 离线经验 = 基础经验速率 × 离线秒数 × 衰减系数 × (1+经验加成)
    // 引擎未实现经验离线计算
  });

  it.skip('§7.12 经验溢出推动升级 [引擎未实现]', () => {
    // Play: 经验溢出推动升级时等级正确提升
    // 引擎未实现经验溢出升级
  });

  it.skip('§7.12 升级奖励邮件自动发放 [引擎未实现]', () => {
    // 引擎未实现升级奖励邮件
  });
...
describe('v9.0 E2E — §7.15 邮件暂存队列补发顺序与完整性', () => {
  ...
  it.skip('§7.15 暂存队列FIFO(先进先出) [引擎未实现]', () => {
    // 引擎未实现邮件暂存队列
  });

  it.skip('§7.15 暂存上限20封硬限制 [引擎未实现]', () => {
    // 引擎未实现邮件暂存队列
  });

  it.skip('§7.15 超过20封丢弃+通知 [引擎未实现]', () => {
    // 引擎未实现邮件暂存溢出处理
  });
...
describe('v9.0 E2E — §7.18 快照降级→邮件通知联动', () => {
  ...
  it.skip('§7.18 降级处理同时触发弹窗+邮件双通道通知 [引擎未实现]', () => {
    // 引擎未实现快照降级通知
  });

  it.skip('§7.18 连续多次快照丢失不重复发送邮件 [引擎未实现]', () => {
    // 引擎未实现快照降级通知
  });
...
describe('v9.0 E2E — §7.23 离线领土变化视觉标记', () => {
  ...
  it.skip('§7.23 金色脉冲"新"标签(24h) [UI层测试]', () => {
    // UI层验证
  });

  it.skip('§7.23 红色脉冲"失"标签(12h) [UI层测试]', () => {
    // UI层验证
  });
  ...
  it.skip('§7.23 视觉标记性能(≥30fps) [UI层测试]', () => {
    // UI层验证
  });
...
describe('v9.0 E2E — §7.26 离线→声望→活动积分三角闭环', () => {
  ...
  it.skip('§7.26 声望升级后加成立即影响后续计算 [引擎未实现]', () => {
    // 引擎未实现声望升级联动
  });
