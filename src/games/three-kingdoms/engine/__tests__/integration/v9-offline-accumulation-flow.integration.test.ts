...
describe('v9.0 累积计算 — §7.19 离线领土攻城行为', () => {
  ...
  it.todo('§7.19 攻城失败损失30%出征兵力 [引擎未实现]', () => {
    // 引擎未实现攻城失败惩罚
  });
...
describe('v9.0 累积计算 — §7.20 离线科技研究完成→解锁', () => {
  ...
  it.todo('§7.20 科技完成后产出加成立即生效 [引擎未实现]', () => {
    // Play: 科技完成后产出加成立即生效
    // 引擎未实现科技完成后的产出更新联动
  });

  it.todo('§7.20 本次离线收益使用下线时快照的加成系数 [引擎未实现]', () => {
...
describe('v9.0 累积计算 — §7.24 离线科技连续完成→产出速率顺序更新', () => {
  ...
  it.todo('§7.24 产出速率按完成时间顺序更新 [引擎未实现]', () => {
    // Play: 逐项验证产出速率变化
    // 引擎未实现科技完成后的产出速率顺序更新
  });

  it.todo('§7.24 快照一致性: 本次离线收益使用下线时快照 [引擎未实现]', () => {
    // 引擎未实现快照一致性机制
  });
...
describe('v9.0 累积计算 — §7.26 离线→声望→活动积分三角闭环', () => {
  ...
  it.todo('§7.26 离线声望升级→活动加成更新 [引擎未实现]', () => {
    // Play: 声望升级后加成立即生效
    // 引擎未实现声望升级联动
  });
