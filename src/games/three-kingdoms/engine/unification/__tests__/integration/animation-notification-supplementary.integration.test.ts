...
describe('§7 补充验证 — 动画降级+通知系统+边界条件 集成测试', () => {
  ...
  describe('§7.1 动画降级', () => {
    ...
    beforeEach(() => {
      ...
      gfxManager.init(createRealDeps());
    });
  ...
  describe('§7.5 未实现功能', () => {
    it('动画降级：低画质下动画时长可通过auditor审查偏差', () => {
      // GraphicsQualityManager 低画质限制30fps，AnimationAuditor 审查动画合规性
      const auditor = new AnimationAuditor();
      const gfx = new GraphicsQualityManager();
      gfx.init(createRealDeps());
      gfx.setPreset(GraphicsPreset.Low);
      const presetConfig = gfx.getPresetConfig();
      expect(presetConfig.frameRateLimit).toBe(30);

      // 注册一个超过低画质帧率限制的动画
      auditor.registerAnimation('test_anim', 'transition', 500, 'ease-out');
    ...
    it('通知系统：FPS低于阈值可检测告警级别', () => {
      // PerformanceMonitor 提供 getFPSAlertLevel 检测 FPS 告警
      const monitor = new PerformanceMonitor();
      monitor.init(createRealDeps());
      monitor.start();
      // 模拟低 FPS：注入低帧率采样
      for (let i = 0; i < 10; i++) {
        monitor.update(0.1); // 10fps
      }
      const alertLevel = monitor.getFPSAlertLevel();
      expect(['critical', 'warning', 'good', 'excellent']).toContain(alertLevel);
    });

    it('视觉一致性：综合报告包含动画+配色检查结果', () => {
      // VisualConsistencyChecker.generateReport() 生成综合报告
      const checker = new VisualConsistencyChecker();
      checker.init(createRealDeps());
      const report = checker.generateReport();
      expect(report).toBeDefined();
      expect(report).toHaveProperty('animationReport');
      expect(report).toHaveProperty('colorReport');
