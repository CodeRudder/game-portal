...
describe('集成测试: 手机端适配 (Play §1.7, §11.1-11.2)', () => {
  ...
  describe('§11.2 战斗系统手机端', () => {
    it.skip('技能按钮尺寸≥44×44px（UI层验证，引擎无此概念）', () => {
      // [过时/UI层] 引擎层无按钮尺寸概念，需在 React 组件测试中验证
      // 已用 it.skip 标注，无需在引擎层处理
    });

    it.skip('大招时停期间按钮突出显示（UI层验证）', () => {
      // [过时/UI层] 倍速切换按钮易触达属于 UI 交互层，需在组件测试中验证
    });

    it.skip('倍速切换按钮易触达（UI层验证）', () => {
      // [过时/UI层] 战斗全屏布局属于 UI 渲染层，需在组件测试中验证
    });
  });
  ...
  describe('§1.7 手机端战斗布局', () => {
    ...
    it.skip('触摸优化（UI层验证）', () => {
      // [过时/UI层] 触摸优化属于 UI 交互层，需在组件测试中验证
    });
  });
