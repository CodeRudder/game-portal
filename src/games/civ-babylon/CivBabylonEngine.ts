/**
 * 巴比伦文明放置游戏 (Civ Babylon) — 主引擎 v3.0
 *
...
export interface CivBabylonSaveState {
...
export class CivBabylonEngine extends IdleGameEngine {
  ...
  protected onInit(): void {
    ...
    this.stages = new StageSystem<StageDef>(DYNASTIES, 'sumer');
  ...
  public serialize(): CivBabylonSaveState {
    const heroStates: Record<string, { level: number; exp: number }> = {};
    for (const h of HEROES) {
      const s = this.units.getState(h.id);
      if (s?.unlocked) heroStates[h.id] = { level: s.level, exp: s.exp };
    }
    return {
      resources: { ...this.res },
  ...
  public deserialize(d: CivBabylonSaveState): void {
  ...
  public doPrestige(): void {
    const total = (this.res.grain || 0) + (this.res.clay || 0) + (this.res.copper || 0) + (this.res.silver || 0);
    ...
    this.res.clay = (this.res.clay || 0) * ret;
    this.res.copper = (this.res.copper || 0) * ret;
    this.res.silver = (this.res.silver || 0) * ret;

    // 重置建筑和阶段
    this.bldg.reset(true);
    this.stages.loadState({ currentStageId: 'sumer' });

    this.stats.increment('totalPrestiges');
    this.ftSys.add('声望转生！神眷降临！', 0.5, 0.4, { style: { color: COLOR_THEME.accentGold, fontSize: 24 } });
    this.emit('stateChange');
  }
  ...
  private makeStatDefs() {
    const ids = [
      'totalGrain', 'totalClay', 'totalCopper', 'totalSilver', 'totalFavor',
  ...
  private drawPrestige(ctx: CanvasRenderingContext2D, w: number, sy: number, _h: number): void {
    ...
    y += 30;
    ctx.fillStyle = pv.canPrestige ? 'rgba(201,169,110,0.2)' : COLOR_THEME.panelBg;
  ...
  private drawFooter(ctx: CanvasRenderingContext2D, w: number, h: number): void {
    ...
    ctx.fillText('[Space]点击 [↑↓]选择 [Enter]购买 [R]声望 [T]科技 [H]英雄 [Esc]返回', w / 2, h - 12);
    ctx.textAlign = 'left';
  }
