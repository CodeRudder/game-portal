 ...
 *
 * @module components/idle/panels/campaign/SweepPanel
 */
...
const SweepPanel: React.FC<SweepPanelProps> = ({
  ...
  const renderStars = () => {
    ...
    for (let i = 0; i < MAX_STARS; i++) {
      elements.push(
        <span
          key={i}
          className={`tk-sweep-stage-star ${i < stars ? 'tk-sweep-stage-star--filled' : ''}`}
        >
          ★
        </span>,
      );
  ...
  const renderSweepResult = () => {
    ...
    return (
      <div className="tk-sweep-results" data-testid="sweep-panel-result">
  ...
  const renderAutoPushResult = () => {
    ...
    return (
      <div className="tk-sweep-results" data-testid="sweep-panel-autopush-result">
        <div className="tk-sweep-results-title">🚀 自动推图完成</div>
        <div className="tk-sweep-results-summary">
  ...
  return (
    <div className="tk-sweep-panel-overlay" role="dialog" aria-modal="true" aria-label="扫荡面板" data-testid="sweep-panel-overlay">
      <div className="tk-sweep-panel">
        {/* 标题栏 */}
        <div className="tk-sweep-panel-header">
          <h3 className="tk-sweep-panel-title">
            {chapterName} · {stageName} · 扫荡
          </h3>
          <button className="tk-sweep-panel-close" onClick={onClose} aria-label="关闭">
            ✕
          </button>
        </div>

        {/* 内容区 */}
        <div className="tk-sweep-panel-body">
          {/* 关卡信息卡 */}
          <div className="tk-sweep-stage-card">
            <span className="tk-sweep-stage-icon">⚔️</span>
            <div className="tk-sweep-stage-meta">
              <div className="tk-sweep-stage-name-text">{stageName}</div>
          ...
          {canSweep && (
            <div className="tk-sweep-count-section">
              ...
              <div className="tk-sweep-count-control">
                <button className="tk-sweep-count-btn" onClick={handleDecrease} disabled={sweepCount <= 1} aria-label="减少次数">−</button>
                <div className="tk-sweep-count-display">{sweepCount}</div>
                <button className="tk-sweep-count-btn" onClick={handleIncrease} disabled={sweepCount >= maxSweep} aria-label="增加次数">+</button>
                <button className="tk-sweep-max-btn" onClick={handleMax}>MAX {maxAffordable}</button>
              </div>
            </div>
          )}
        ...
        {/* 底部操作 */}
        <div className="tk-sweep-panel-footer">
...
SweepPanel.displayName = 'SweepPanel';
