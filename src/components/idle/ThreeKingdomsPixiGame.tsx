...
import { Formation, FORMATION_EFFECTS, COUNTER_TABLE, TROOP_TYPE_NAMES, COUNTER_ADVANTAGE, COUNTER_DISADVANTAGE } from '@/games/three-kingdoms/BattleStrategy';
...
function LevelDetailModal({
  ...
  const [selectedFormation, setSelectedFormation] = useState<string>('standard');

  // 阵型选项配置（与 BattleStrategy.Formation 同步）
  const formationOptions = [
    { key: Formation.STANDARD, label: '普通', bonus: '均衡' },
    { key: Formation.OFFENSIVE, label: '攻击', bonus: '攻击+20%' },
    { key: Formation.DEFENSIVE, label: '防御', bonus: '防御+20%' },
    { key: Formation.AMBUSH, label: '伏击', bonus: '首回合+50%' },
  ];
    ...
    <div style={{
      ...
      {canAttack && (
        <div style={{ marginBottom: 12 }}>
          <div style={{ color: CT.accentGold, fontSize: 13, marginBottom: 6, fontFamily: "'KaiTi', 'STKaiti', 'Noto Serif SC', serif" }}>⚔️ 选择阵型</div>
          <div style={{ display: 'flex', gap: 6 }}>
            {formationOptions.map(f => {
              const isSelected = selectedFormation === f.key;
              return (
                <button key={f.key} onClick={() => setSelectedFormation(f.key)}
                  style={{
                    flex: 1, padding: '6px 4px',
                    background: isSelected
                      ? 'linear-gradient(135deg, #6b3a1f, #8b5a2f)'
                      : 'rgba(74, 44, 23, 0.5)',
                    border: isSelected ? '2px solid #f0c060' : '1px solid #8b6914',
                    borderRadius: 6, color: isSelected ? '#f0c060' : '#b8a080',
                    cursor: 'pointer', fontSize: 11, textAlign: 'center',
                    transition: 'all 0.2s ease',
                  }}>
                  <div style={{ fontSize: 14 }}>{FORMATION_EFFECTS[f.key as Formation].icon}</div>
                  <div style={{ fontWeight: isSelected ? 'bold' : 'normal' }}>{f.label}</div>
                  <div style={{ fontSize: 9, color: '#a08060', marginTop: 2 }}>{f.bonus}</div>
                </button>
              );
            })}
          </div>
          {/* 阵型说明 */}
          <div style={{ fontSize: 10, color: CT.textDim, marginTop: 4, textAlign: 'center' }}>
            {FORMATION_EFFECTS[selectedFormation as Formation].description}
          </div>
        </div>
      )}
      ...
      <div style={{
        ...
        background: 'rgba(74, 44, 23, 0.3)', borderRadius: 6,
        border: '1px solid rgba(139, 105, 20, 0.3)',
      }}>
        <div style={{ color: CT.accentGold, fontSize: 12, marginBottom: 4, fontFamily: "'KaiTi', 'STKaiti', 'Noto Serif SC', serif" }}>🎯 兵种克制</div>
        <div style={{ fontSize: 11, color: '#b8a080', lineHeight: 1.8 }}>
          {Object.entries(COUNTER_TABLE).map(([attacker, defender]) => (
            <span key={attacker}>
              {TROOP_TYPE_NAMES[attacker as keyof typeof TROOP_TYPE_NAMES]} → 克制 → {TROOP_TYPE_NAMES[defender as keyof typeof TROOP_TYPE_NAMES]}
              {'　'}
            </span>
          ))}
        </div>
        <div style={{ fontSize: 10, color: '#8b6914' }}>
          克制+{Math.round(COUNTER_ADVANTAGE * 100)}%伤害 | 被克制{Math.round(COUNTER_DISADVANTAGE * 100)}%伤害
        </div>
      </div>
    ...
    <div style={{
      ...
      {Array.isArray(result.rounds) && result.rounds.length > 0 && (
        <div style={{ marginBottom: 12 }}>
          ...
          {/* 阵型与克制提示 */}
          <div style={{
            fontSize: 10, color: CT.textDim, marginBottom: 6, padding: '4px 8px',
            background: 'rgba(74, 44, 23, 0.2)', borderRadius: 4,
            borderLeft: '3px solid #8b6914',
          }}>
            <span>⚔️ 阵型效果与兵种克制已生效</span>
            <span style={{ marginLeft: 8, color: '#8b6914' }}>克制+30% | 被克制-20%</span>
          </div>
          <div style={{
...
const GeneralCard = ({
  ...
  return (
    <div
      className={`tk-general-card ${general.unlocked ? 'tk-general-card--unlocked' : 'tk-general-card-locked'} ${isSelected ? 'tk-general-card--selected' : ''} ${general.unlocked ? `tk-general-card--rarity-${general.rarity}` : ''} ${general.unlocked ? `tk-general-card--faction-${general.faction}` : ''} ${general.unlocked ? `tk-general-card--hero-${general.id.replace(/[^a-z0-9]/gi, '').toLowerCase()}` : ''}`}
...
export default function ThreeKingdomsPixiGame() {
  ...
  // ─── 地图动态事件 ──────────────────────────────────────────
  const [mapEvent, setMapEvent] = useState<import('@/games/three-kingdoms/ThreeKingdomsEngine').MapEvent | null>(null);

  // ─── NPC 互动气泡 ──────────────────────────────────────────
  const [npcBubble, setNpcBubble] = useState<{ text: string; generalId: string } | null>(null);
  ...
  // ─── 武将详情 Tab ──────────────────────────────────────────
  const [heroDetailTab, setHeroDetailTab] = useState<'info' | 'biography' | 'bonds'>('info');
  ...
  const [buildingLevelUps, setBuildingLevelUps] = useState<Array<{ id: number; buildingId: string; level: number }>>([]);
  const buildingLevelUpIdRef = useRef(0);
  ...
  const [resourceDeltas, setResourceDeltas] = useState<Array<{ id: number; resourceId: string; text: string }>>([]);
  const resourceDeltaIdRef = useRef(0);

  useEffect(() => {
    const prev = prevResourcesRef.current;
    const curr: Record<string, number> = {};
    const changed = new Set<string>();
    const newDeltas: Array<{ id: number; resourceId: string; text: string }> = [];
    for (const r of resources) {
      ...
      if (diff > 0 && prevAmt > 0) {
        ...
        changed.add(r.id);
        // 资源增量飘字效果
        const deltaId = ++resourceDeltaIdRef.current;
        newDeltas.push({ id: deltaId, resourceId: r.id, text: `+${fmt(diff)}` });
        // 800ms后移除飘字
        setTimeout(() => {
          setResourceDeltas(prev => prev.filter(d => d.id !== deltaId));
        }, 800);
      }
    }
    ...
    if (newDeltas.length > 0) {
      setResourceDeltas(prev => [...prev, ...newDeltas]);
    }
  }, [resources, showFloatText]);

  // ─── 建筑升级等级追踪（飘字 + 升级完成动画） ──────────────
  useEffect(() => {
    const prev = prevBuildingLevelsRef.current;
    const curr: Record<string, number> = {};
    const levelUps: Array<{ id: number; buildingId: string; level: number }> = [];
    for (const b of buildings) {
      curr[b.id] = b.level;
      const prevLv = prev[b.id] ?? 0;
      if (b.level > prevLv && prevLv > 0) {
        const upId = ++buildingLevelUpIdRef.current;
        levelUps.push({ id: upId, buildingId: b.id, level: b.level });
        // 触发闪光动画
        triggerBuildingFlash(b.id);
        // 1s后移除飘字
        setTimeout(() => {
          setBuildingLevelUps(prev => prev.filter(u => u.id !== upId));
        }, 1000);
      }
    }
    prevBuildingLevelsRef.current = curr;
    if (levelUps.length > 0) {
      setBuildingLevelUps(prev => [...prev, ...levelUps]);
    }
  }, [buildings, triggerBuildingFlash]);
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      if (engineRef.current) {
        const recruitedIds = engineRef.current.getRecruitedGeneralIds();
        if (recruitedIds.length > 0) {
          const gId = recruitedIds[Math.floor(Math.random() * recruitedIds.length)];
          const dialogues = [
            '主公，末将愿效犬马之劳！',
            '近日操练兵马，士气高昂！',
            '听闻敌军蠢蠢欲动...',
          ...
          setTimeout(() => setNpcBubble(null), 4000);
        }
      }
    }, 15000); // 每15秒
    return () => clearInterval(interval);
      ...
      <header className="tk-header-ancient" style={{
        ...
        <div className="tk-resource-panel-gold" style={{
          ...
          {resources.map(r => (
            ...
            >
              <span className={changedResources.has(r.id) ? 'tk-resource-icon-updated' : ''} style={{ display: 'inline-flex', alignItems: 'center' }}>
              ...
              {/* 资源增量飘字 */}
              {resourceDeltas.filter(d => d.resourceId === r.id).map(d => (
                <span key={d.id} className={`tk-resource-delta tk-resource-delta--${r.id}`}>
                  {d.text}
                </span>
              ))}
            </div>
          ...
          {activeMapSubTab === 'building' && (
            <div style={{ flex: 1, overflowY: 'auto' }}>
              {buildings.map(b => {
                ...
                // 查找此建筑的升级飘字
                const levelUp = buildingLevelUps.find(u => u.buildingId === b.id);
                return (
                  <div
                    key={b.id}
                  ...
                  >
                    {/* 建筑升级飘字 */}
                    {levelUp && (
                      <span className="tk-building-level-up-text">
                        ⬆ Lv.{levelUp.level}
                      </span>
                    )}
                    {/* 卡片上部：图标 + 名称/等级 */}
                    ...
                    {/* 可升级时显示升级进度条动画 */}
                    {b.canUpgrade && b.level > 0 && (
                      <div className="tk-building-upgrade-bar" style={{ marginTop: 4, paddingLeft: 48 }}>
                        <div
                          className="tk-building-upgrade-fill tk-building-upgrade-fill--shimmer"
                          style={{ '--tk-progress-target': `${levelProgress * 100}%` } as React.CSSProperties}
                        />
                      </div>
                    )}
                  </div>
          ...
          {scene === 'map' && (
            <div className="tk-scene-panel">
              ...
              <div className="tk-scene-production">
                ...
                {resources.map(r => (
                  <div key={r.id} className="tk-scene-prod-row" style={{ position: 'relative' }}>
                    <span className={`tk-scene-prod-icon ${changedResources.has(r.id) ? 'tk-resource-icon-updated' : ''}`}>
                      <ResourceIcon resourceId={r.id} size={16} />
                    </span>
                    <span className="tk-scene-prod-name">{r.name}</span>
                    <span className={`tk-scene-prod-rate ${changedResources.has(r.id) ? 'tk-resource-value-updated' : ''}`}>
                      +{fmt(r.perSecond)}/秒
                    </span>
                    {/* 场景面板资源增量飘字 */}
                    {resourceDeltas.filter(d => d.resourceId === r.id).map(d => (
                          ...
                          return (
                            <div key={node.id} className="tk-tech-node--appear" style={{ animationDelay: `${idx * 0.05}s` }}>
                              ...
                              >
                                {/* R16: 研究中 — 外圈旋转进度环 */}
                                {isResearching && (
                                  <div className="tk-tech-ring" style={{
                                    position: 'absolute', top: -4, left: -4, right: -4, bottom: -4,
                                    pointerEvents: 'none', zIndex: 3,
                                  }}>
                                    <svg viewBox="0 0 40 40" style={{ width: '100%', height: '100%' }}>
                                      <circle cx="20" cy="20" r="18" fill="none" stroke="rgba(212,160,48,0.15)" strokeWidth="1.5" />
                                      <circle cx="20" cy="20" r="18" fill="none" stroke="rgba(212,160,48,0.7)" strokeWidth="1.5"
                                        strokeDasharray={`${node.progress * 113} ${113 - node.progress * 113}`}
                                        strokeLinecap="round" transform="rotate(-90 20 20)" className="tk-tech-ring--breathe" />
                                    </svg>
                                  </div>
                                )}
                                {/* R16: 已解锁 — 金色涟漪扩散（首次完成时） */}
                                {isCompleted && (
                                  <div className="tk-tech-unlock-ripple" />
                                )}
                                {/* 已解锁 — 金色标记 + 粒子环绕 */}
                                {isCompleted && (
                                  <>
            ...
            <div style={{ flex: 1 }}>
              ...
              <div className="tk-scroll-panel" style={{ marginTop: 8 }}>
                ...
                <div className="tk-detail-tabs">
                  ...
                  <button
                    className={`tk-detail-tab-btn ${heroDetailTab === 'bonds' ? 'tk-detail-tab-btn--active' : ''}`}
                    onClick={() => setHeroDetailTab('bonds')}
                  >
                    ◆ 羁绊
                  </button>
                </div>
                ...
                {heroDetailTab === 'info' ? (
                  ...
                  <div style={{ marginTop: 4 }}>
                    {(() => {
                      const engine = engineRef.current;
                      if (!engine || !selectedHero) return null;
                      const allBonds = engine.getGeneralBonds(selectedHero.id);
                      const activatedIds = new Set(engine.bondSystem.serialize().activatedBonds);
                      return allBonds.length > 0 ? (
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                          {allBonds.map((bond: any) => {
                            const isActive = activatedIds.has(bond.id);
                            return (
                              <div key={bond.id} style={{
                                padding: '8px 10px',
                                background: isActive ? 'rgba(212, 165, 116, 0.15)' : 'rgba(0,0,0,0.2)',
                                border: `1px solid ${isActive ? '#8B7355' : 'rgba(139,115,85,0.3)'}`,
                                borderRadius: 6, opacity: isActive ? 1 : 0.6,
                              }}>
                                <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 2 }}>
                                  <span style={{ color: isActive ? '#f0c060' : '#8a7a6a', fontWeight: 'bold', fontSize: 12 }}>
                                    {isActive ? '🤝' : '🔒'} {bond.name}
                                  </span>
                                  {bond.tier && (
                                    <span style={{ fontSize: 9, color: '#b87333', border: '1px solid #b87333', borderRadius: 3, padding: '0 4px' }}>
                                ...
                                <div style={{ fontSize: 10, color: '#b8a080', marginBottom: 2 }}>
                                  {bond.generalIds?.map((gid: string) => {
                                    const g = heroes.find((h: any) => h.id === gid);
                                    return g?.name || gid;
                                  }).join(' + ')}
                                </div>
                                <div style={{ fontSize: 10, color: '#a08060' }}>{bond.description}</div>
                                {isActive && bond.bonus && Object.keys(bond.bonus).length > 0 && (
                                  <div style={{ fontSize: 9, color: '#6aaf6a', marginTop: 2 }}>
                                    {Object.entries(bond.bonus).map(([k, v]) => `${k}: +${v}`).join('  ')}
                                  </div>
                                )}
                              </div>
                        ...
                        <div style={{ color: '#8a7a6a', fontSize: 12, textAlign: 'center', padding: 16 }}>
                          此武将暂无羁绊关系
                        </div>
                      );
                    })()}
        ...
        >
          <div
            style={{
              ...
              padding: 20,
              color: '#e8d5b7',
              boxShadow: '0 4px 24px rgba(0,0,0,0.6), 0 0 40px rgba(192,144,80,0.2)',
              cursor: 'default',
            }}
            onClick={(e) => e.stopPropagation()}
          >
            ...
            {mapEvent.rewards && (
              <div style={{
                ...
                🎁 奖励：{Object.entries(mapEvent.rewards).map(([k, v]) => {
                  const labelMap: Record<string, string> = {
                    gold: '💰金币', grain: '🌾粮草', fame: '⭐声望',
                    troops: '⚔️兵力', techPoints: '📜科技点', intel: '🔍情报',
                  };
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              <button
                onClick={() => {
                  engineRef.current?.resolveMapEvent(mapEvent.id);
                  addToast(`${mapEvent.title} — 事件已处理！`, 'success');
                  setMapEvent(null);
                }}
                style={{
                  padding: '8px 16px',
                  background: 'linear-gradient(135deg, #6b3a1f, #8b5a2f)',
      ...
      {npcBubble && (
        <div
          style={{
            ...
            maxWidth: 400,
            textAlign: 'center',
            boxShadow: '0 2px 12px rgba(0,0,0,0.4), 0 0 20px rgba(212,165,116,0.15)',
