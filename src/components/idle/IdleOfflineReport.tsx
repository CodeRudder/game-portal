...
export default function IdleOfflineReport({
  ...
  if (entries.length === 0) {
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm" data-testid="offline-report">
        <div className="mx-4 w-full max-w-sm rounded-2xl border border-gp-accent/30 bg-gp-card p-6 text-center">
          ...
          <button
            onClick={onCollect}
            data-testid="offline-report-collect"
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm" data-testid="offline-report">
      <div className="mx-4 w-full max-w-sm animate-slide-up rounded-2xl border border-gp-accent/30 bg-gp-card p-6">
        ...
        <div className="mb-4 text-center">
          ...
          <p className="mt-1 text-xs text-gray-400" data-testid="offline-report-duration">
            离线时长: <span className="text-gp-gold">{formatDuration(report.offlineMs)}</span>
          </p>
        </div>

        {/* 资源列表 */}
        <div className="mb-4 flex flex-col gap-2" data-testid="offline-report-resources">
          {entries.map(([resId, amount]) => (
            <div
              key={resId}
              className="flex items-center justify-between rounded-lg bg-white/5 px-3 py-2"
              data-testid={`offline-report-resource-${resId}`}
            >
