/**
 * components/idle/HeroesMightPixiGame.tsx — 英雄无敌 PixiJS React 组件
 *
 * 奇幻主题渲染组件，使用 HeroesMightScene 替代通用 IdleScene。
 * 紫金配色，冒险地图，英雄队伍，技能面板。
 *
    ...
    const init = async () => {
      try {
        ...
        if (strategy) {
          ...
          const baseStrategy = RenderStrategyRegistry.get('heroes-might');
  ...
  return (
    ...
    >
      {ready && (
        <div
          style={{
            ...
            color: fps >= 55 ? '#34d399' : fps >= 30 ? '#fbbf24' : '#c0392b',
            fontSize: 11,
      ...
      {error && (
        ...
        >
          <div style={{ fontSize: 16, fontWeight: 'bold' }}>⚠️ 魔法阵启动失败</div>
