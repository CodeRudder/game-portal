/**
 * components/idle/AgeOfEmpiresPixiGame.tsx — 帝国时代 PixiJS React 组件
 *
 * 帝国主题渲染组件，使用 AgeOfEmpiresScene 替代通用 IdleScene。
 * 棕金配色，城镇地图，资源采集，科技升级树。
 *
    ...
    const init = async () => {
      try {
        ...
        if (strategy) {
          ...
          const baseStrategy = RenderStrategyRegistry.get('age-of-empires');
  ...
  return (
    ...
    >
      {ready && (
        <div
          style={{
            ...
            color: fps >= 55 ? '#6ab04c' : fps >= 30 ? '#f0932b' : '#c0392b',
            fontSize: 11,
      ...
      {error && (
        ...
        >
          <div style={{ fontSize: 16, fontWeight: 'bold' }}>⚠️ 帝国建设失败</div>
