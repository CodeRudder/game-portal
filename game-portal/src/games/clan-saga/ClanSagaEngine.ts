...
export interface ClanSagaSaveState {
...
/** 兼容旧导出名 */
export type ClanSagaState = ClanSagaSaveState;
...
export class ClanSagaEngine extends IdleGameEngine {
  ...
  protected onInit(): void {
    ...
    this.stages = new StageSystem<StageDef>(DYNASTIES, 'small_clan');
  ...
  public serialize(): ClanSagaSaveState {
    const heroState: Record<string, { level: number; exp: number }> = {};
    for (const h of HEROES) {
      const s = this.units.getState(h.id);
      if (s?.unlocked) heroState[h.id] = { level: s.level, exp: s.exp };
    }
    return {
      resources: { ...this.res },
  ...
  public deserialize(d: ClanSagaSaveState): void {
  ...
  public doPrestige(): void {
    const total = (this.res.grain || 0) + (this.res.silk || 0) + (this.res.stone || 0);
    ...
    this.stages.loadState({ currentStageId: 'small_clan' });

    this.stats.increment('totalPrestiges');
    this.ftSys.add('血脉觉醒！先祖庇佑！', 0.5, 0.4, { style: { color: COLOR_THEME.accentGold, fontSize: 24 } });
    this.emit('stateChange');
  }
  ...
  private checkStage(): void {
    ...
    if (can) {
      ...
      this.ftSys.add(`家族晋升：${next.name}`, 0.5, 0.3, { style: { color: COLOR_THEME.accentGold, fontSize: 22 } });
    }
  }
  ...
  private makeStatDefs() {
    const ids = [
      'totalGrain', 'totalSilk', 'totalStone', 'totalBloodline',
  ...
  private drawHeader(ctx: CanvasRenderingContext2D, w: number): void {
    ...
    const tabs = [
      ...
      { id: 'heroes' as ActivePanel, label: '👥族人' },
      { id: 'tech' as ActivePanel, label: '💡传承' },
      { id: 'prestige' as ActivePanel, label: '🩸觉醒' },
    ];
  ...
  private drawPrestige(ctx: CanvasRenderingContext2D, w: number, sy: number, _h: number): void {
    ...
    y += 30;
    ctx.fillStyle = pv.canPrestige ? 'rgba(192,57,43,0.2)' : COLOR_THEME.panelBg;
    rr(ctx, w / 2 - 70, y, 140, 32, 6); ctx.fill();
    ctx.fillStyle = pv.canPrestige ? COLOR_THEME.accentGold : COLOR_THEME.textDim;
    ctx.font = '13px sans-serif'; ctx.textAlign = 'center';
    ctx.fillText(pv.canPrestige ? '执行觉醒 (R)' : '资源不足', w / 2, y + 20);
    ctx.textAlign = 'left';
  }
  ...
  private drawHeroes(ctx: CanvasRenderingContext2D, w: number, sy: number, h: number): void {
    ...
    for (let i = 0; i < Math.min(HEROES.length, vis); i++) {
      ...
      ctx.fillText(`政${s.administration} 武${s.military} 文${s.culture}`, 18, iy + 50);
      ctx.textAlign = 'right';
      if (!own) {
        const costStr = Object.entries(o.recruitCost).map(([k, v]) => `${v}${k === 'grain' ? '🌾' : '🧵'}`).join(' ');
  ...
  private drawFooter(ctx: CanvasRenderingContext2D, w: number, h: number): void {
    ...
    ctx.fillText('[Space]点击 [↑↓]选择 [Enter]购买 [R]觉醒 [T]传承 [U]族人 [Esc]返回', w / 2, h - 12);
    ctx.textAlign = 'left';
  }
