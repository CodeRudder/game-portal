...
> **生成日期**: 2025-04-27 | **项目**: 三国霸业 | **版本**: v1.1
> **更新日期**: 2026-06-09 | **数据来源**: PRD文档 + UI设计文档 + 引擎源码 + 测试文件
> **v1.1 变更**：数值对齐（PRD→引擎）、招贤令经济循环修复、突破石/技能书获取途径补充、缺失数值设计补全
...
> **变更日期**: 2026-06-09 | **变更类型**: PRD数值对齐 + 经济循环修复 + 缺失设计补全

### 已修复的问题

| 编号 | 原问题描述 | 修复内容 | 影响章节 |
|------|-----------|---------|---------|
| R1-PRD | PRD星级系数公式 `1+(星级-1)×0.10` 与引擎 `STAR_MULTIPLIERS` 查表不一致 | PRD修正为查表法，与引擎对齐 | HER-1 战力计算 |
| R2-PRD | PRD突破节点为 Lv.10/20/30/40，引擎实际为 30→40→50→60→70 四阶突破 | PRD修正为引擎的四阶突破配置 | HER-3 突破系统 |
| R3-PRD | PRD品质体系缺少品质倍率和枚举映射 | 补充品质倍率列和枚举值列 | HER-1 品质体系 |
| R4-PRD | PRD升星表缺少铜钱消耗列 | 新增铜钱消耗列，与引擎 `STAR_UP_GOLD_COST` 对齐 | HER-5 升星消耗 |
| R5-PRD | 招贤令经济循环断裂（产出极低、获取途径缺失） | 新增 HER-7 招贤令经济设计章节 | HER-7（新增） |
| R6-PRD | 突破石无获取途径 | 新增 HER-8 突破石获取途径章节 | HER-8（新增） |
| R7-PRD | 技能书无获取途径 | 新增 HER-9 技能书获取途径章节 | HER-9（新增） |

### 新增的功能点（PRD定义）

| 编号 | 功能点 | PRD章节 | 引擎状态 | 优先级 |
|------|--------|---------|---------|--------|
| NEW-1 | 招贤令被动产出（0.003/秒） | HER-7 | ❌ 待实现 | P0 |
| NEW-2 | 新手礼包（首次登录赠送50招贤令） | HER-7 | ❌ 待实现 | P0 |
| NEW-3 | 日常任务奖励招贤令（每日5个） | HER-7 | ❌ 待实现 | P1 |
| NEW-4 | 商店购买招贤令（100铜钱/个，日限30） | HER-7 | ❌ 待实现 | P1 |
| NEW-5 | 关卡首通奖励招贤令（3~5个） | HER-7 | ❌ 待实现 | P1 |
| NEW-6 | 活动奖励招贤令（10~20个） | HER-7 | ❌ 待实现 | P2 |
| NEW-7 | 离线收益招贤令（50%效率） | HER-7 | ❌ 待实现 | P1 |
| NEW-8 | 突破石关卡掉落 | HER-8 | ❌ 待实现 | P0 |
| NEW-9 | 突破石商店购买（500铜钱/个，日限20） | HER-8 | ❌ 待实现 | P1 |
| NEW-10 | 突破石成就奖励 | HER-8 | ❌ 待实现 | P2 |
| NEW-11 | 突破石联盟商店（200联盟币/个，周限30） | HER-8 | ❌ 待实现 | P2 |
| NEW-12 | 技能书日常任务掉落 | HER-9 | ❌ 待实现 | P1 |
| NEW-13 | 技能书远征掉落 | HER-9 | ❌ 待实现 | P1 |
| NEW-14 | 技能书活动奖励 | HER-9 | ❌ 待实现 | P2 |
| NEW-15 | 技能书联盟商店（300联盟币/本，周限10） | HER-9 | ❌ 待实现 | P2 |
| NEW-16 | 技能CD减少机制（2→3级：CD -1回合） | HER-4 | ❌ 待实现 | P2 |
| NEW-17 | 技能额外效果（4→5级解锁） | HER-4 | ❌ 待实现 | P2 |
| NEW-18 | 觉醒技能效果表 | HER-4 | ❌ 待实现 | P1 |
| NEW-19 | 碎片合成武将所需碎片数量表 | HER-5 | ✅ 引擎已有 `SYNTHESIZE_REQUIRED_FRAGMENTS` | — |
| NEW-20 | 碎片溢出→铜钱转化比率（1碎片=100铜钱） | HER-5 | ✅ 引擎已有 `FRAGMENT_TO_GOLD_RATE` | — |

### PRD数值对齐确认

| 数值项 | 原PRD值 | 引擎实际值 | 对齐状态 |
|--------|---------|-----------|:-------:|
| 招募概率（普通池） | [60,30,8,2,0]% | [60,30,8,2,0]% | ✅ 一致 |
| 招募概率（高级池） | [20,40,25,13,2]% | [20,40,25,13,2]% | ✅ 一致 |
| 保底阈值（普通池） | 无硬保底 | hardPityThreshold=Infinity | ✅ 一致 |
| 保底阈值（高级池） | 100次必出Legendary+ | hardPityThreshold=100 | ✅ 一致 |
| 战力权重 | ATK×2.0,CMD×1.5,INT×2.0,POL×1.0 | POWER_WEIGHTS一致 | ✅ 一致 |
| 等级系数 | 1+等级×0.05 | LEVEL_COEFFICIENT_PER_LEVEL=0.05 | ✅ 一致 |
| 星级系数 | 1+(星级-1)×0.10（线性） | 查表[1.0,1.15,1.35,1.6,2.0,2.5] | ✅ PRD已修正 |
| 招募消耗（普通） | 招贤榜×1 | recruitToken, amount=1 | ✅ 一致 |
| 招募消耗（高级） | 求贤令×100 | recruitToken, amount=100 | ✅ 一致 |
| 突破节点 | Lv.10/20/30/40 | 30→40→50→60→70（四阶） | ✅ PRD已修正 |
| 升星铜钱消耗 | 未定义 | [0,5000,10000,20000,50000,100000] | ✅ PRD已补充 |

### 新增PRD章节索引

| 章节 | 标题 | 说明 |
|------|------|------|
| HER-7 | 招贤令经济设计 | 招贤令获取途径、每日产出估算、消耗产出平衡 |
| HER-8 | 突破石获取途径 | 突破石获取途径、需求总量 |
| HER-9 | 技能书获取途径 | 技能书获取途径、需求总量 |
| HER-10 | 数值权威来源索引 | 所有数值的引擎文件来源集中索引 |

---

*武将功能点实现检查清单 v1.1 | 更新于 2026-06-09*
