...
| 审查维度 | 标准 |
|---------|------|
| 文件行数 | 业务代码≤500行（预警400行），测试≤1000行（预警800行） |
| 单一职责 | 每个文件可一句话描述 |
...
核心原则：通过 `tick()` 时间加速推进游戏状态，测试用例按 Play 文档的流程 ID 组织，便于追溯。运行时机：Phase 4 引擎逻辑修复后回归，Phase 6 封版前全量通过。

**禁止**：直接设置资源/等级/解锁状态（捏造数据）、绕过招募/升级等系统直接注入对象、跳过前置条件检查、用 `addXxxDirectly()` 类方法绕过游戏逻辑。
