...
状态: 🔴 确认为真实P0（升级回P0）
源码验证:
  - engine-save.ts SaveContext: 无map相关字段
  - buildSaveData(): 无map子系统serialize调用
  - applySaveData(): 无map子系统deserialize调用
  - ThreeKingdomsEngine.ts:570-572: mapState/territoryState/siegeState 仅在buildSaveCtx()中通过getState()导出
  - 但buildSaveData()不消费这些字段
影响: 保存/加载游戏后，Map模块8个子系统状态全部丢失
R2结论: 🔴 升级为P0-024，需FIX-714
```
...
### NEW-P0-024: engine-save完全缺失Map子系统序列化
```
来源: R1 P0-024降级 → R2源码验证 → 确认升级
严重度: P0（数据丢失）
影响: 保存游戏后Map模块所有状态丢失（领土归属、地标等级、攻城历史、驻防配置、事件状态、冷却时间戳）
修复方案: FIX-714
  - SaveContext添加map子系统引用
  - buildSaveData()添加map序列化
  - applySaveData()添加map反序列化
  - GameSaveData类型添加map字段
```
...
| 维度 | R1 | R2 | 变化 |
|------|-----|-----|------|
| P0活跃 | 22 | 1 | -21 (FIX-701~713) +1 (P0-024升级) |
