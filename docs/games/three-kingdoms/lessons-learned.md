...
### LL-009: 更多菜单导航链路从未端到端验证

**严重程度**: P0  
**发现时间**: 2026-04-29  
**影响范围**: 所有FLOW测试 + ACC-01 + FLOW-23/25

**问题**: 整个测试体系中没有任何测试验证过「主界面 → 点击更多▼ → 菜单展开 → 选择功能 → 面板打开」的完整玩家视觉路径。FLOW-23全部38个用例render次数=0，FLOW-25使用自建TabStateManager类而非真实组件。

**根因**: 
1. 方法论缺失 — 没有"玩家视觉路径"规则
2. 组件隔离测试惯性 — 每个组件单独测试，跨组件集成点从未被覆盖
3. 测试命名误导 — 名为"点击展开"实为传入prop

**修复**: 
- 修正ACC-01-15/16、FLOW-23-30/31为真实DOM交互
- 重写FLOW-25删除自建TabStateManager
- 新增NAV-01导航链路端到端测试
- 补充5条方法论规则到test-coverage-improvement-plan.md


---

### LL-010: assertVisible 假断言 — 函数名暗示验证可见性但实际只检查 inline style

**严重程度**: P1  
**发现时间**: 2026-04-29  
**影响范围**: 全部 ACC/FLOW 测试（29个文件、304处引用）

**问题**: assertVisible() 函数名暗示验证了「元素视觉可见性」，但实际只检查：
- 元素存在于 DOM（非 null）
- inline style 的 display !== none
- inline style 的 visibility !== hidden

**未检查的关键场景**：
- CSS class 造成的 overflow: hidden 裁切（如下拉面板、折叠区域）
- CSS class 造成的 height: 0 / max-height: 0
- opacity: 0（通过 class）
- clip-path: inset(100%) 等视觉隐藏手段

**导致的假通过**: 某些元素被 CSS class 裁切（如 overflow: hidden; height: 0），在真实浏览器中不可见，但 assertVisible() 通过了测试，因为 inline style 没有设置 display: none。

**根因**: 
1. 函数命名不精确 — Visible 暗示视觉可见性，但 jsdom 无法计算 CSS class 的级联效果
2. 缺少文档说明 — 函数没有注释说明其局限性
3. 测试环境限制 — jsdom 的 getComputedStyle 不反映真实浏览器计算结果

**修复**: 
- 将 assertVisible() 重命名为 assertInDOM()，准确反映其行为（检查元素存在 + 未被 inline style 隐藏）
- 新增 assertVisuallyRenderable() 函数，增加 opacity/clip-path 检查，但明确标注 jsdom 限制
- 在关键位置（下拉面板、展开区域）添加 TODO: 需要Playwright E2E验证视觉可见性 注释
- 更新所有 304 处引用

**教训**: 
1. **函数命名必须精确反映实际行为** — 如果只能检查存在于DOM，就不要叫Visible
2. **视觉可见性验证必须用真实浏览器** — jsdom 无法验证 CSS class 造成的裁切/隐藏
3. **诚实标注测试局限性** — 比假装能验证更安全；用注释明确说明此处无法验证
