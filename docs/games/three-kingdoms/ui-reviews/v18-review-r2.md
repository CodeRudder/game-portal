...
| # | 问题 | 严重度 | 说明 |
|---|------|--------|------|
| W1 | UI层无guide组件 | P2 | 未找到 UI 层 guide 相关组件文件，guide 引擎层完整但缺少 React/Vue 渲染层对接 |
| W2 | e2e 测试依赖运行时 | P2 | e2e 脚本需要 `GAME_BASE_URL` 指向运行中的游戏实例，CI 环境需额外配置 |
| W3 | 移动端跳过按钮位置未验证 | P2 | e2e 测试验证了 Skip 可点击，但未检查按钮是否被遮挡或超出安全区域 |
