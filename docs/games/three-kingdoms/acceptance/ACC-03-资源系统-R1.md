...
> ⚠️ [需求变更] 资源增加脉冲动画已移除（避免持续动画影响注意力）。此条目中"资源数值增加时的视觉脉冲反馈"不再需要。容量警告脉冲/闪烁动画（urgent/full）保留。

| 属性 | 详情 |
|------|------|
| **优先级** | ~~P2（功能点#12 资源产出脉冲动画）~~ [已移除] |
| **期望** | ~~资源数值增加时有视觉脉冲反馈~~ [需求变更：已移除] |
| **实际** | CSS `.tk-res-item--pulse` 中 `animation: resource-pulse 0.4s ease-out;` **被注释掉**。JS 中 `setPulsing(true)` 逻辑存在但无视觉效果 |
| **修复建议** | ~~取消注释 `.tk-res-item--pulse` 中的 `animation` 属性~~ [不再需要，资源增加脉冲已移除] |

### ❌ ACC-03-02 补充 — 容量警告动画被注释

> ⚠️ [需求变更] 资源增加脉冲动画已移除，但容量警告脉冲/闪烁动画（urgent/full）保留，仍需启用。
...
| **修复建议** | 取消注释相关 animation 属性（容量警告动画保留），或确认是性能优化后的有意为之 |
...
> ⚠️ [需求变更] 资源增加脉冲动画已移除（避免持续动画影响注意力）。容量警告脉冲/闪烁动画（urgent/full）保留，仍需启用。
